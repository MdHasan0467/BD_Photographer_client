...
const MyReviewsLoader = ({ review, handleDelete }) => {
	const { _id, message, name,  serviceTitle, serviceImg, time, image } = review;

	return (
		<div>
			<section className='body-font group'>
					...
					<div className=''>
						<div className='p-4  w-full'>
							...
							<div className='flex'>
									...
									<div className='mx-5'>
									<div className='time text-start text-sm text-white'>
									{time?.slice(0, 15)}
									</div>
									<div className='time text-start text-sm text-white'>
									{time?.slice(16, 25)}
								...
								<div className='img overflow-hidden rounded-lg'>
									<img className='w-full h-40 object-cover opacity-75 group-hover:opacity-50 group-hover:scale-110 transition duration-300 ease-in-out' src={serviceImg} alt='' />
								</div>
								<h1 className='text-white'>
									<span className='font-bold text-emerald-600 mx-2'>PhotoType:</span>
									{serviceTitle}
								</h1>
								<p className='leading-relaxed text-white my-10'>
									<span className='font-bold text-emerald-600 mx-2'>Review:</span>
									{message}
								</p>
